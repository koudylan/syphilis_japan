...
data {
  ...
  // 
  int<lower=1> W;                      
  array[W] real<lower=0, upper=1> scen_mult;
}
...
parameters {
  ...
  real<lower=0, upper=1> rep1;              // reporting rate (2016-2019)
  real<lower=0, upper=1> rep2;              // reporting rate (2020-21)
  real<lower=0, upper=1> rep3;              // reporting rate (2022-)
...
for (t in 25:T) {
  ...
  array[T] real rep;
  for (t in 1:16) rep[t] = rep1;         // 2016-2019
  for (t in 17:24) rep[t] = rep2;
  for (t in 25:T) rep[t] = rep3;
  
  // time-varying CS risk among viable deliveries
  array[T] real r;
  for (t in 1:24) r[t] = 0.15;         // 2016-2019
  for (t in 25:T) r[t] = rrisk * 0.15; // 2022-...
